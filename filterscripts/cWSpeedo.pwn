...
Float:GetVehicleSpeed_moi(vehicleid)
{
	new
	    Float:x,
	    Float:y,
...
public OnPlayerConnect(playerid)
{
	...
	cWspeedo[playerid][7] = CreatePlayerTextDraw(playerid, 572.509460, 407.528869, "Xang");
...
public OnPlayerStateChange(playerid, newstate, oldstate)
{
	if(newstate == PLAYER_STATE_DRIVER)
	{
		...
		{
	        new vstr[30];
			format(vstr, sizeof(vstr), "%s", GetVehicleName_moi(vehicleid));
...
public OnPlayerUpdate(playerid)
{
	...
	if(IsPlayerInAnyVehicle(playerid) && GetPlayerState(playerid) == PLAYER_STATE_DRIVER && !IsAbicycle(vehicle)) // Making sure the player is in a vehicle as driver
	{
	    if(PlayerSpeedo[playerid] == 0)
	    {
			...
			new speed[24];
			carfuel[vehicle] = PlayerVehicleInfo[playerid][vehicle][pvFuel];
			format(speed, sizeof(speed), "%.0f", GetVehicleSpeed_moi(vehicle));
...
//-----------------------------------------------------------------------------//
CMD:thaydoispeedo(playerid, params[])
{
	#pragma unused params
	if(PlayerSpeedo[playerid] == 0)
	{
        PlayerSpeedo[playerid] = 1;
        SendClientMessage(playerid, -1, "Ban da chuyen Speedo qua dinh dang MP/H");
	}
	else
	{
        PlayerSpeedo[playerid] = 0;
		SendClientMessage(playerid, -1, "Ban da chuyen Speedo qua dinh dang KM/H");
	}
	return 1;
}
