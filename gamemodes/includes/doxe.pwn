...
hook OnGameModeInit()
{
        ...
        wTuning3[5] = TextDrawCreate(88.000000, 252.000000, "red");
        ...
        wTuning3[6] = TextDrawCreate(88.000000, 271.000000, "remove_neon");
        TextDrawBackgroundColor(wTuning3[6], 255);
        TextDrawFont(wTuning3[6], 2);
        TextDrawLetterSize(wTuning3[6], 0.300000, 1.000000);
        TextDrawColor(wTuning3[6], -1);
        TextDrawSetOutline(wTuning3[6], 0);
        TextDrawSetProportional(wTuning3[6], 1);
        TextDrawSetShadow(wTuning3[6], 0);
        TextDrawUseBox(wTuning3[6], 1);
        TextDrawBoxColor(wTuning3[6], 0);
        TextDrawTextSize(wTuning3[6], 170.000000, 10.000000);
        TextDrawSetSelectable(wTuning3[6], 0);
        //------------------------------------------------------------------------------
        //
        //
        //------------------------------------------------------------------------------
...
CMD:doxe(playerid, params[]){
        if(!IsNhanVienSuaXe(playerid))
        {
            return SendClientMessage(playerid, COLOR_GREY, "Ban khong phai la nhan vien Sua Xe");
        }
        if(IsPlayerInRangeOfPoint(playerid, 5.0, 1935.5713,-1994.6171,13.6192) || IsPlayerInRangeOfPoint(playerid, 5.0, 1935.6754,-1985.8162,13.6192))
        {
        ...
        {
            return SendClientMessage(playerid, COLOR_GREY, "Ban khong o vi tri Do Xe");
        }
...
stock SavePlayerVehicleMods(playerid)
{
        for(new d = 0 ; d < MAX_PLAYERVEHICLES; d++)
        {
                for(new i = 0; i < MAX_PLAYERS; i++)
                {
                        if(IsPlayerInVehicle(playerid, PlayerVehicleInfo[i][d][pvId]))
                        {
