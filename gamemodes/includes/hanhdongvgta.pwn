...
hook OnPlayerClickTextDraw(playerid, Text:clickedid)
{
	...
	{
		ShowPlayerDialog(EditID, DIALOG_HANHDONG, DIALOG_STYLE_INPUT, "Save Animation", " {c4c4c4}Memorable comment... (optional)", "Next", "Close");
	}
	else
	{
...
hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	switch(dialogid)
	{
		case DIALOG_HANHDONG: if(response)
		{
			new string[120];
			GetAnimationName(save_Name, animlib, 32, animname, 32);			
		...
		case DIALOG_HANHDONG2: if(response)
		{
			switch(listitem)
			{
				case 0: ShowPlayerDialog(EditID, DIALOG_HANHDONG3, DIALOG_STYLE_INPUT, "Speed", "{c4c4c4}Specify speed. Default: 4.1", "Next", "Back");
				case 1: a_Cycle ^= true, ShowSettingDialog();
				case 2: a_LockPos ^= true, ShowSettingDialog();
				case 3: a_Freeze ^= true, ShowSettingDialog();
				case 4: ShowPlayerDialog(EditID, DIALOG_HANHDONG4, DIALOG_STYLE_INPUT, "Time", "{c4c4c4}Specify time. Default: 0", "Next", "Back");
				case 5: a_Forcesync ^= true, ShowSettingDialog();
			}
			ClearAnimations(EditID, a_Forcesync);
		}
		case DIALOG_HANHDONG3:
		{
			if(response)
			{
				new Float:speed = floatstr(inputtext);
				if(speed < 0.1000 || speed > 4.10000) return ShowPlayerDialog(EditID, DIALOG_HANHDONG3, DIALOG_STYLE_INPUT, "Speed", "{ff0000}Error value!     {c4c4c4}0.1 — 4.1\nSpecify speed. Default: 4.1", "Next", "Back");
				a_Speed = speed + 0.001;				
			}
			ShowSettingDialog();
		}
		case DIALOG_HANHDONG4:
		{
			if(response)
			{
				new time = strval(inputtext);
				if(time < 0 || time > 255) return ShowPlayerDialog(EditID, DIALOG_HANHDONG4, DIALOG_STYLE_INPUT, "Time", "{ff0000}Error value!\n{c4c4c4}Specify time. Default: 0", "Next", "Back");
				a_Time = time;				
			}
			ShowSettingDialog();
		}
...
stock ShowSettingDialog()
{
	...
	ShowPlayerDialog(EditID, DIALOG_HANHDONG2, DIALOG_STYLE_LIST, "Setting", string, "Select", "Close");
}
CMD:hanhdongvgta(playerid, params[])
{
