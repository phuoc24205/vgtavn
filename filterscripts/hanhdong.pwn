...
{
    if(GetPVarType(playerid, "PlayerCuffed") || GetPVarType(playerid, "CuocGoiAnimtion") || GetPVarType(playerid, "Injured") || GetPVarType(playerid, "IsFrozen") || GetPVarInt(playerid, "Hospital")) {
   		SendClientMessage(playerid, COLOR_GRAD2, "Khong the lam dieu do tai thoi diem nay!");
   		return 0;
	}
    if(IsPlayerInAnyVehicle(playerid) == 1)
    {
		SendClientMessage(playerid, COLOR_GRAD2, "Khong the lam dieu nay tren xe.");
		return 0;
	}
	return 1;
}
...
IsAbleVehicleAnimation(playerid)
{
    ...
    {
		SendClientMessage(playerid, COLOR_GRAD2, "Khong the lam nay tren xe.");
		return 0;
	}
	return 1;
}
...
CMD:piss(playerid, params[])
{
	...
	{
		 SendClientMessage(playerid, COLOR_WHITE, "ban phai doi 20 s sau moi co the su dung lai");
	}
    return 1;
}
...
CMD:fallover(playerid, params[])
{
	...
	case 1:
	{
        PlayAnimEx(playerid, "KNIFE", "KILL_Knife_Ped_Die", 4.0, 0, 1, 1, 1, 0, 1);
        Used[playerid] = 1;
    }
	case 2:
	{
	    PlayAnimEx(playerid, "PED", "KO_shot_face", 4.0, 0, 1, 1, 1, 0, 1);
	    Used[playerid] = 1;
    }
	case 3:
	{
        PlayAnimEx(playerid, "PED", "KO_shot_stom", 4.0, 0, 1, 1, 1, 0, 1);
        Used[playerid] = 1;
    }
	case 4:
	{
	...
	case 5:
	{
	    PlayAnimEx(playerid, "PED", "BIKE_fall_off", 4.1, 0, 1, 1, 1, 0, 1);
	    Used[playerid] = 1;
    }
...
CMD:diadiemdua(playerid, params[])
{
    SetPlayerCheckpoint(playerid,1762.094, 2721.580 , 10.83, 4.0);
    return ;
}
