...
public OnPlayerUpdate( playerid )
{
	...
	new veh1 = GetPlayerVehicleID(playerid),model = GetVehicleModel(veh1);
	if(IsPlayerInAnyVehicle(playerid) && GetPlayerState(playerid) == PLAYER_STATE_DRIVER) 
	{ 
		if(model != 460 && model != 520 ||model != 519 ||model != 553 ||model != 577 ||model != 592 ||model != 593 ||model != 513 ||model !=512 ||model !=511 ||model != 476 )
		{
    		...
    		{ 
			    ...
			    { 
				    ...
				    SendClientMessage(playerid, 0xFF0000FF, "Hack fly car"); 
				    //format(pstring, 128, " %s has been banned from the server. Reason:Vehicle Speed Hack", GetPlayerName(playerid)); 
				    //SendClientMessageToAll(0xFF0000FF, pstring); 
...
public OnPlayerEnterVehicle(playerid, vehicleid, ispassenger)
{
	...
	{
		g_fSpeedCap[ playerid ] = 0.18 ;
	}
	else{
		g_fSpeedCap[ playerid ] = 0 ;
	}
