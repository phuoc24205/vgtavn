...
#include <YSI\y_hooks>
#define MAX_ZOMBIES 500 // change this to whatever number you want
//zombie vgta
new ZombiesTimer;
...
public FCNPC_OnDeath(npcid, killerid, reason)
{
    ...
    SetTimerEx("OnPlayerKillZombie", 120000, 0, "ii",killerid,npcid);
    ...
    for(new i = 0 ; i < MAX_PLAYERS; i++)
    {
        if(PickupZombie[i] == -1)
        {
            ...
            switch(rand)
            {
                ...
                // case 94..97: 
                // {
                //     PickupZombie[i] = CreatePickup(19793, 1, x, y, z, 0);//hotquet
...
timer ZombieMove[500](zombieid)
{
    ...
    foreach(Player, playerid)
    {
        ...
        if(IsPlayerInRangeOfPoint(zombieid,1.5,xa,ya,za))// here playerid not zombieid
        {
            ZombieIsAttack[zombieid] = 2;


            FCNPC_MeleeAttack(zombieid,100,true);
            break;
        }
        else if(IsPlayerInRangeOfPoint(zombieid,10.0,xa,ya,za))
        {
            ...
            FCNPC_GoTo(zombieid,xa,ya,za,FCNPC_MOVE_TYPE_RUN,0.7);
            break;
        }
...
stock FCNPC_GoToPlayerEx(npcid, playerid, Float:dist, Float:rangle, movetype = FCNPC_MOVE_TYPE_RUN)
{
    ...
    FCNPC_GoTo(npcid,xa,ya,za,movetype,0.8);
    return 1;
}
    ...
    for(new i = 0; i < MAX_PLAYERS; i++)
    {
        if(pickupid == PickupZombie[i])
        {
            if(PickupZombieLoai[i] == 0)//tien
            {
                ...
                new Random = random(2000)+1000;
                GivePlayerMoney(playerid, Random);
                PickupZombie[i] = -1;
                PickupZombieLoai[i] = -1;
                
                new string[128];
                format(string, sizeof(string), "Ban nhan duoc %d$ tu viec giet chet zombie Covid-19 .",Random);
            ...
            // {
            //     new Random = random(200)+100;
            //     PlayerInfo[playerid][pMats] += Random;
            //     PickupZombie[i] = -1;
            //     PickupZombieLoai[i] = -1;
            //     DestroyPickup(PickupZombie[i]);
            //     new string[128];
            //     format(string, sizeof(string), "Ban nhan duoc %d vat lieu tu viec giet chet zombie Covid-19 .",Random);
            //     SendClientMessage(playerid, 0xe3844dFF, string);
            //     return 1;
            // }
